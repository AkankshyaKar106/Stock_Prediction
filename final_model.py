...
def load_lstm_model():
    try:
        lstm_model = load_model("D:/Qwegle/Qwegle/Algorithmic_Trading/lstm.h5", custom_objects={"mse": MeanSquaredError()})
        lstm_model.compile(optimizer="adam", loss="binary_crossentropy", metrics=["accuracy"])  
        with open('D:/Qwegle/Qwegle/Algorithmic_Trading/tokenizer1.pkl', 'rb') as f:
...
def load_rf_model():
    try:
        ...
        with open('D:/Qwegle/Qwegle/Algorithmic_Trading/sentiment_scaler1.pkl', 'rb') as f:
...
def fetch_bse_sensex_tickers():
    ...
    return fetch_tickers(url, table_index=2, ticker_column=0, suffix_check=".BO")
...
def fetch_stock_data(ticker, prediction_date):
    try:
        ...
        start_with_offset = (pd.to_datetime(prediction_date) - pd.DateOffset(days=100)).strftime('%Y-%m-%d')
        
        for suffix in [".NS", ".BO", ""]:
            try:
                stock = yf.Ticker(ticker + suffix)
                data = stock.history(start=start_with_offset, end=prediction_date)
                ...
                if not data.empty:
                    ...
                    for col in required_columns:
                        if col not in data.columns:
                            st.warning(f"Missing column {col} in data for {ticker}")
                            return None
...
def compute_technical_indicators(stock_data):
    ...
    try:
        ...
        df['BB_Upper'], df['BB_Middle'], df['BB_Lower'] = talib.BBANDS(close_array)
...
def predict_next_hour_price(stock_data, sentiment_score, rf_model, scaler, sentiment_scaler, target_scaler):
    try:
        feature_columns = [
        ...
        volatility = stock_data['Returns'].ewm(span=10, min_periods=10).std().iloc[-1] * np.sqrt(6.5)
        ...
        for period in [1, 2, 3, 4, 6, 8]:  # Hours instead of days
            stock_data[f'{period}h_return'] = stock_data['Close'].pct_change(periods=period)
        ...
        sentiment_scaled = sentiment_scaler.transform(np.array([[sentiment_score]]))
        combined_features = np.hstack([scaled_features, sentiment_scaled])
        
        raw_prediction = rf_model.predict(combined_features)[0]
        
        if target_scaler is not None:
            predicted_price = target_scaler.inverse_transform([[raw_prediction]])[0][0]
        else:
        ...
        if abs(predicted_change) > 0.5 * hourly_volatility:  
            dampening_factor = np.exp(-3 * abs(predicted_change) / hourly_volatility)  
            predicted_change *= dampening_factor
        
        capped_change = np.clip(predicted_change, -max_hourly_move/last_close, max_hourly_move/last_close)
        ...
        if abs((final_predicted_price - ema5_price) / ema5_price) > ma_boundary:
            final_predicted_price = ema5_price * (1 + np.sign(final_predicted_price - ema5_price) * ma_boundary)
...
def calculate_difference_and_signal(predicted_price, stock_data, prediction_time=None):
    try:
        last_close = stock_data["Close"].iloc[-1]
...
def predict_next_day_price(stock_data, sentiment_score, rf_model, scaler, sentiment_scaler, target_scaler):
    try:
        feature_columns = [
            "Open", "High", "Low", "Close", "Volume",
            "SMA_20", "SMA_50", "RSI", "MACD", "BB_Middle", "ATR", "ROC", "MOM"
        ...
        stock_data['Returns'] = stock_data['Close'].pct_change()
        volatility = stock_data['Returns'].ewm(span=10, min_periods=10).std().iloc[-1] * np.sqrt(252)
        daily_volatility = volatility / np.sqrt(252)
        
        stock_data['EMA_5'] = stock_data['Close'].ewm(span=5, adjust=False).mean()
        
        for period in [1, 2, 3, 5, 7, 10]:
            stock_data[f'{period}d_return'] = stock_data['Close'].pct_change(periods=period)
        ...
        if abs(predicted_change) > 0.5 * daily_volatility:  
            dampening_factor = np.exp(-3 * abs(predicted_change) / daily_volatility)  
            predicted_change *= dampening_factor
        
        capped_change = np.clip(predicted_change, -max_daily_move/last_close, max_daily_move/last_close)
...
def process_ticker_data(ticker, adjusted_datetime, prediction_created_at, stock_data, lstm_model, tokenizer, rf_model, scaler, sentiment_scaler, target_scaler, prediction_type):
    ...
    sentiment_score = calculate_sentiment_score(sentiment_data, lstm_model, tokenizer)
    stock_data['Sentiment_Score'] = sentiment_score
    
    if prediction_type == "Next Day":
        predicted_price = predict_next_day_price(stock_data, sentiment_score, rf_model, scaler, sentiment_scaler, target_scaler)
    else:
        predicted_price = predict_next_hour_price(stock_data, sentiment_score, rf_model, scaler, sentiment_scaler, target_scaler)
    
    if predicted_price is not None:
        signal, reason = calculate_difference_and_signal(predicted_price, stock_data)
...
def get_next_market_datetime(dt, prediction_type="Next Hour"):
    ...
    if ist_dt.weekday() >= 5:
        ...
        if prediction_type == "Next Day":
            ...
            message = f"Market is closed on weekends. Prediction adjusted to Monday opening {ist_dt.strftime('%Y-%m-%d %H:%M')} IST"
        else:
            ist_dt = ist_dt.replace(hour=9, minute=15, second=0, microsecond=0)
    ...
    market_end = ist_dt.replace(hour=15, minute=30, second=0, microsecond=0)
    next_market_start = ist_dt.replace(hour=9, minute=15, second=0, microsecond=0)
    
    if prediction_type == "Next Day":
        ist_dt = (ist_dt + timedelta(days=1)).replace(hour=9, minute=15, second=0, microsecond=0)
...
def save_prediction(prediction_data):
    ...
    try:
        ...
        prediction_created = datetime.strptime(prediction_data['Prediction Created'], '%Y-%m-%d %H:%M:%S IST')
        target_date = datetime.strptime(prediction_data['Target Prediction Date'], '%Y-%m-%d').date()
        target_time = datetime.strptime(prediction_data['Target Prediction Time'], '%H:%M IST').time()
        ...
        c.execute("SELECT * FROM predictions WHERE ticker=? AND prediction_created=?", 
                 (prediction_data['Ticker'], prediction_created))
        saved_record = c.fetchone()
        
        if saved_record:
            st.success(f"Successfully saved prediction for {prediction_data['Ticker']}")
        else:
...
def predict_stock_price():
    ...
    prediction_option = st.radio("Predict for:", ["Next Hour", "Next Day", "Custom Time"])
    ...
    if prediction_option == "Custom Time":
        ...
        if prediction_datetime < current_ist_time:
            st.warning("Selected time is in the past. Please select a future time.")
            return
            
        prediction_type = "Custom Time"
    elif prediction_option == "Next Day":
        today = datetime.now()
        user_selected_date = st.date_input("Select Prediction Date", value=today, min_value=today)

        try:
            prediction_datetime = datetime.combine(user_selected_date, datetime.min.time())
    ...
    adjusted_datetime, market_message = get_next_market_datetime(prediction_datetime, prediction_type)
    ...
    if st.button("Collect Data and Predict"):
                ...
                with st.spinner(f"Computing indicators for {ticker}..."):
                    ...
                    if prediction_data:
                        ...
                        try:
                            ...
                            st.write(f"Successfully saved prediction for {ticker}")
                        except Exception as e:
                            st.error(f"Error saving prediction for {ticker}: {str(e)}")
                            st.write(f"Results for {ticker}:")
                            st.write(prediction_data)
                status.empty()
        ...
        if all_prediction_data:
            ...
            st.write(f"Predictions generated at: {prediction_created_at.strftime('%Y-%m-%d %H:%M:%S IST')}")
