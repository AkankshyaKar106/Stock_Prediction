...
def load_sentiment_data(sentiment_file):
    ...
    sentiment_agg = sentiment_data.groupby(["Ticker"])["Sentiment_Score"].mean().reset_index()
...
def prepare_features(data, sentiment_scaler=None):
    ...
    if sentiment_scaler is not None:
        df['Sentiment_Score'] = sentiment_scaler.transform(df[['Sentiment_Score']])

    return df

def train_model(data, features, target, model_path, scaler_path, target_scaler_path, sentiment_scaler_path):
    x = data[features]
    y = data[target]
    ...
    x_train_scaled = scaler.fit_transform(x_train.drop(columns='Sentiment_Score'))
    x_test_scaled = scaler.transform(x_test.drop(columns='Sentiment_Score'))
    
    x_train_sentiment_scaled = sentiment_scaler.fit_transform(x_train[['Sentiment_Score']])
    x_test_sentiment_scaled = sentiment_scaler.transform(x_test[['Sentiment_Score']])
...
def predict_next_day(rf_model, latest_data, features, scaler, ticker, target_scaler, sentiment_scaler):
    if latest_data.empty:
        return pd.DataFrame()
    
    latest_data_df = latest_data[features].iloc[-1].fillna(0).to_frame().T  
    
    latest_data_scaled = scaler.transform(latest_data_df.drop(columns=['Sentiment_Score']))
    latest_data_sentiment_scaled = sentiment_scaler.transform(latest_data_df[['Sentiment_Score']])
    latest_data_scaled = np.hstack([latest_data_scaled, latest_data_sentiment_scaled])
    
    predicted_price = rf_model.predict(latest_data_scaled)[0]
    predicted_price = target_scaler.inverse_transform([[predicted_price]])[0][0]
    
    today_price = latest_data["Previous_Close"].iloc[-1] if "Previous_Close" in latest_data.columns else None
    signal = "BUY" if today_price and predicted_price > today_price else "SELL" if today_price and predicted_price < today_price else "HOLD"
    reason = f"Predicted price ({predicted_price:.2f}) {'higher' if signal == 'BUY' else 'lower'} than today's price ({today_price:.2f})" if today_price else "Insufficient data"
...
def predict_next_hour(rf_model, latest_data, features, scaler, ticker, target_scaler, sentiment_scaler):
    if latest_data.empty:
        return pd.DataFrame()
    ...
    signal = "BUY" if predicted_price > current_price else "SELL" if predicted_price < current_price else "HOLD"
    reason = f"Predicted next-hour price ({predicted_price:.2f}) {'higher' if signal == 'BUY' else 'lower'} than current price ({current_price:.2f})"
...
if __name__ == "__main__":
    ...
    features = ["Previous_Close", "Open", "High", "Low", "Volume", "SMA_20", "SMA_50", "RSI", "MACD", "BB_Middle", "Sentiment_Score", "ATR", "ROC", "MOM"]
    target = "Close"
    
    rf_model, scaler, target_scaler, sentiment_scaler = train_model(data, features, target, model_path, scaler_path, target_scaler_path, sentiment_scaler_path)
    selected_stock_data = data[data['Ticker'] == selected_ticker]
    prediction_df = predict_next_day(rf_model, selected_stock_data, features, scaler, selected_ticker, target_scaler, sentiment_scaler)
    print(prediction_df)
